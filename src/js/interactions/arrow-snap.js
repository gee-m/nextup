...
export const ArrowSnapMixin = {
    ...
    getArrowEndpoint(sourceTask, targetTask, type) {
        ...
        if (type === 'target') {
            ...
            // Source end (not implemented yet, but structure is here)
            // Would check sourceTask.customSourcePoints[targetTask.id]
    ...
    startArrowDotDrag(dotInfo) {
        ...
        const arrowSelector = `.parent-link[data-child-id="${dotInfo.taskId}"][data-parent-id="${dotInfo.parentId}"]`;
    ...
    renderArrowDotDragVisuals() {
        ...
        const arrowSelector = `.parent-link[data-child-id="${task.id}"][data-parent-id="${parent.id}"]`;
        const arrowLine = document.querySelector(arrowSelector);

        if (arrowLine) {
            // Update arrow endpoint to match dot
            if (arrowLine.tagName === 'line') {
                arrowLine.setAttribute('x2', dotPos.x);
                arrowLine.setAttribute('y2', dotPos.y);
            } else if (arrowLine.tagName === 'path') {
                // For curved arrows, need to update path data
                const x1 = parseFloat(arrowLine.getAttribute('data-x1') || parent.x);
                const y1 = parseFloat(arrowLine.getAttribute('data-y1') || parent.y);

                const dx = dotPos.x - x1;
                const dy = dotPos.y - y1;
                ...
                const midX = (x1 + dotPos.x) / 2;
                const midY = (y1 + dotPos.y) / 2;

                const d = `M ${x1} ${y1} Q ${midX + offsetX} ${midY + offsetY}, ${dotPos.x} ${dotPos.y}`;
    ...
    finishArrowDotDrag() {
        ...
        const arrowSelector = `.parent-link[data-child-id="${this.arrowDotDrag.taskId}"][data-parent-id="${this.arrowDotDrag.relatedTaskId}"]`;
        const arrowLine = document.querySelector(arrowSelector);
        if (arrowLine) {
