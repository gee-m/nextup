...
export const RenderMixin = {
    ...
    renderTaskImage(g, task, rectWidth, rectHeight) {
        try {
            ...
            // Use current image dimensions (can be user-resized)
            const imageWidth = task.imageWidth || 200;
            const imageHeight = task.imageHeight || 150;

            // Check if hovered for preview
            const isHovered = this.hoveredImageTaskId === task.id;

            // For hover preview, use original natural dimensions if available
            let displayWidth = imageWidth;
            let displayHeight = imageHeight;
            if (isHovered && task.imageNaturalWidth && task.imageNaturalHeight) {
                displayWidth = task.imageNaturalWidth;
                displayHeight = task.imageNaturalHeight;
            }

            // Create SVG image element
            const image = document.createElementNS('http://www.w3.org/2000/svg', 'image');
            image.setAttribute('x', -displayWidth / 2);
            image.setAttribute('y', -displayHeight / 2);
            image.setAttribute('width', displayWidth);
            image.setAttribute('height', displayHeight);
            image.setAttribute('href', blobUrl);
            image.setAttribute('preserveAspectRatio', 'xMidYMid meet');
            image.style.pointerEvents = 'all';  // Enable mouse events for hover/click
            image.style.cursor = 'pointer';
            image.dataset.taskId = task.id;
            image.classList.add('task-image');

            // If hovering, show on top with shadow
            if (isHovered) {
                image.style.filter = 'drop-shadow(0 8px 16px rgba(0,0,0,0.3))';
            ...
            if (this.selectedTaskIds.has(task.id) && !isHovered) {
    ...
    /**
     * Open image in full-size modal with zoom controls
     * @param {Object} task - Task with image
     * @param {string} blobUrl - Image blob URL
     */
    openImageModal(task, blobUrl) {
